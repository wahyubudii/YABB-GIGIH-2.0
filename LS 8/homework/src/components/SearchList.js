          ...
          <div key={track.id} className="grid grid-cols-2 text-gray-500 py-4 px-1 hover:bg-gray-900 rounded-lg cursor-pointer">
            ...
            <div className="flex items-center justify-between ml-auto md:ml-0 mx-4">
              ...
              {selectedTracksUri.includes(track.uri) ? (
                <button onClick={() => toggleSelect(track)} className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg mx-4">Selected</button>
              ) : (
                <button onClick={() => toggleSelect(track)} className="bg-white hover:opacity-90 text-black px-5 py-2 rounded-lg mx-4">Select</button>
              )}
            </div>
          </div>
        )}
